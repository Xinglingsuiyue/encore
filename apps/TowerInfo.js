...
export class TowerInfo extends plugin {
    ...
    async downloadTowerIcons(e) {
        ...
        if (!fs.existsSync(TOWER_ICON_DIR)) fs.mkdirSync(TOWER_ICON_DIR, { recursive: true })
        ...
        for (let i = 0; i < ids.length; i++) {
            ...
            try {
                const { readLocalDetail } = await import('./EncoreSync.js')
                rawData = readLocalDetail('toa', phase)
            } catch {}
            if (!rawData) {
                try {
                    const res = await fetch(`https://api.encore.moe/zh-Hans/toa/${phase}`, {
            ...
            for (const url of urls) {
                if (await this._downloadIcon(url, TOWER_ICON_DIR)) downloaded++
            }
            if (downloaded > 0) ok++
            else skip++
        }

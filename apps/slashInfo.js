...
export class WhiWaInfo extends plugin {
    ...
    async downloadWhiWaIcons(e) {
        ...
        if (!fs.existsSync(WHIWA_ICON_DIR)) fs.mkdirSync(WHIWA_ICON_DIR, { recursive: true })
        ...
        for (let i = 0; i < total; i++) {
            ...
            if (detail.buffItems && Array.isArray(detail.buffItems)) {
                for (const bi of detail.buffItems) {
                    const it = bi.item || {}
                    this._addUrl(urls, urlFix(it.icon))
                }
            }
            ...
            for (const url of urls) {
                if (await this._downloadIcon(url, WHIWA_ICON_DIR)) downloaded++
            }
            if (downloaded > 0) ok++
            else skip++
        }
